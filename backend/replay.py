...
class ReplayBundle:
    ...
    def to_dict(self) -> dict[str, object]:
        self.validate()
        return {"schema": self.schema, "run_id": self.run_id, "request_fingerprint": self.request_fingerprint, "checkpoints": [checkpoint.__dict__ for checkpoint in self.checkpoints], "immutable_inputs_digest": self.immutable_inputs_digest, "artifact_digests": list(self.artifact_digests)}
...
def build_replay_bundle(*, run_id: str, request_fingerprint: str, immutable_inputs: object, checkpoints: tuple[ReplayCheckpoint, ...], artifact_digests: tuple[str, ...] = ()) -> ReplayBundle:
    bundle = ReplayBundle("research-replay-bundle/v1", run_id, request_fingerprint, checkpoints, digest_payload(immutable_inputs), artifact_digests)
    bundle.validate()
    return bundle
...
def replay_from_checkpoint(bundle: ReplayBundle, *, expected_inputs: object, last_completed_stage: str | None = None) -> str:
    ...
    if last_completed_stage not in [checkpoint.stage for checkpoint in bundle.checkpoints]:
