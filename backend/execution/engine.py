...
def _transition_terminal(run: ResearchRun, status: ResearchLifecycle) -> ResearchRun:
    if run.status not in {ResearchLifecycle.PLANNED, ResearchLifecycle.RUNNING}:
...
def transition_research(
    ...
    if (
        run.status in {ResearchLifecycle.PLANNED, ResearchLifecycle.RUNNING}
        and target in TERMINAL_RESEARCH_LIFECYCLES
    ):
...
def summarize_research(run: ResearchRun) -> dict[str, Any]:
    ...
    corroborated = sum(
        1
        for _, result in run.verified_claims
