...
class ResearchLifecycle(StrEnum):
    """Canonical lifecycle used by execution and downstream receipts."""
...
class ResearchRun:
    ...
    def __post_init__(self) -> None:
        try:
            lifecycle = (
                self.status
                if isinstance(self.status, ResearchLifecycle)
                else ResearchLifecycle(self.status)
            )
...
def verify_and_add_claim(
    ...
    """Verify a claim against observations and prior claims."""
...
def _transition_terminal(run: ResearchRun, target: ResearchLifecycle) -> ResearchRun:
    if run.status is not ResearchLifecycle.RUNNING:
        raise ValueError(f"invalid or unsupported run transition: {run.status} -> {target}")
    if target not in TERMINAL_RESEARCH_LIFECYCLES:
        raise ValueError(f"invalid or unsupported run transition: {run.status} -> {target}")
    return ResearchRun(
        run.run_id,
        run.contract,
...
def transition_research(run: ResearchRun, status: ResearchLifecycle | str) -> ResearchRun:
    """Apply the canonical lifecycle algebra without a second run authority."""
    try:
    ...
    if run.status is ResearchLifecycle.RUNNING and target is ResearchLifecycle.COMPLETED:
        return complete_research(run, success=True)
    if run.status is ResearchLifecycle.RUNNING and target is ResearchLifecycle.FAILED:
        return complete_research(run, success=False)
    if run.status is ResearchLifecycle.RUNNING and target in {
        ResearchLifecycle.PARTIAL,
        ResearchLifecycle.BLOCKED,
        ResearchLifecycle.CANCELLED,
    }:
...
def summarize_research(run: ResearchRun) -> dict[str, Any]:
    ...
    corroborated = sum(
        1 for _, result in run.verified_claims if result.status == ClaimStatus.CORROBORATED
    )
    contradicted = sum(
        1 for _, result in run.verified_claims if result.status == ClaimStatus.CONTRADICTED
    )
    findings = [
        {
            "claim": claim.text,
