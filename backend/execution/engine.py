...
def _transition_terminal(run: ResearchRun, status: ResearchLifecycle) -> ResearchRun:
    ...
    return ResearchRun(run.run_id, run.contract, run.plan, status, run.observations, run.claims, run.verified_claims, run.started_at, datetime.now(timezone.utc))
...
def start_research(run: ResearchRun) -> ResearchRun:
    ...
    return ResearchRun(run.run_id, run.contract, run.plan, ResearchLifecycle.RUNNING, run.observations, run.claims, run.verified_claims, datetime.now(timezone.utc), run.completed_at)


def add_observation(run: ResearchRun, obs: Observation, budget: ResourceBudget) -> ResearchRun:
    budget.consume_evidence()
    return ResearchRun(run.run_id, run.contract, run.plan, run.status, run.observations + (obs,), run.claims, run.verified_claims, run.started_at, run.completed_at)


def add_claim(run: ResearchRun, claim: Claim) -> ResearchRun:
    return ResearchRun(run.run_id, run.contract, run.plan, run.status, run.observations, run.claims + (claim,), run.verified_claims, run.started_at, run.completed_at)
...
def complete_research(run: ResearchRun, success: bool = True) -> ResearchRun:
    ...
    return _transition_terminal(run, ResearchLifecycle.COMPLETED if success else ResearchLifecycle.FAILED)
...
def summarize_research(run: ResearchRun) -> dict[str, Any]:
    if not run.verified_claims:
        return {"run_id": run.run_id, "status": run.status, "question": run.contract.question, "observations": len(run.observations), "claims_verified": 0, "corroborated_claims": 0, "contradicted_claims": 0, "findings": []}
    ...
    findings = [{"claim": claim.text, "status": result.status, "supporting_evidence": len(result.supporting_evidence), "independent_corroboration": result.independent_corroboration_count, "reasons": list(result.reasons)} for claim, result in run.verified_claims]
    return {"run_id": run.run_id, "status": run.status, "question": run.contract.question, "observations": len(run.observations), "claims_verified": len(run.verified_claims), "corroborated_claims": corroborated, "contradicted_claims": contradicted, "findings": findings}
