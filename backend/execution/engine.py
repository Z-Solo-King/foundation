...
class ResearchRun:
    ...
    def __post_init__(self) -> None:
        try:
            value = self.status if isinstance(self.status, ResearchLifecycle) else ResearchLifecycle(self.status)
        except (TypeError, ValueError) as exc:
            raise ValueError(f"invalid run status: {self.status!r}") from exc
...
def _terminalize(run: ResearchRun, status: ResearchLifecycle) -> ResearchRun:
...
def complete_research(run: ResearchRun, success: bool = True) -> ResearchRun:
    return _terminalize(run, ResearchLifecycle.COMPLETED if success else ResearchLifecycle.FAILED)
...
def transition_research(run: ResearchRun, status: ResearchLifecycle | str) -> ResearchRun:
    try:
        ...
        raise ValueError(f"invalid run status: {status!r}") from exc
    ...
    if run.status is ResearchLifecycle.RUNNING and target in TERMINAL_RESEARCH_LIFECYCLES:
        return _terminalize(run, target)
