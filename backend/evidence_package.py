...
class EvidencePackage:
    ...
    def validate(self, trust_key: bytes) -> None:
        ...
        if len(self.artifact_digest) != 64 or any(c not in "0123456789abcdef" for c in self.artifact_digest.lower()):
...
def parse_trusted_package(payload: object, trust_key: bytes) -> EvidencePackage:
    ...
    p = EvidencePackage(
        ...
        source_versions=tuple(str(x) for x in payload.get("source_versions", ())),
        evidence_spans=tuple(str(x) for x in payload.get("evidence_spans", ())),
        claims=tuple(x for x in payload.get("claims", ()) if isinstance(x, dict)),
