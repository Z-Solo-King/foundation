...
class WorkflowEvidenceReceipt:
    ...
    def validate(self) -> None:
        ...
        if not all(isinstance(value, str) and value.strip() for value in (self.workflow, self.revision, self.run_id)):
        ...
        if any(state not in valid for state in states):
    ...
    def aggregate_state(self) -> ExecutionState:
        ...
        if any(state == "FAILED" for state in states):
            return "PARTIAL" if any(state == "COMPLETE" for state in states) else "FAILED"
        if any(state == "BLOCKED" for state in states):
            return "BLOCKED"
        if any(state in {"UNKNOWN", "NOT_ATTEMPTED"} for state in states):
            return "PARTIAL" if any(state == "COMPLETE" for state in states) else "UNKNOWN"
        return "COMPLETE"
