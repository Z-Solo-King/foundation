...
def choose_next_lane(
    *,
    ...
    eligible_lanes = [
        ...
        for item in lanes
        if item.authorized
        and item.name not in failed
        and not (item.requires_browser and not policy.allow_browser)
        and (not policy.allowed_capabilities or item.capability in policy.allowed_capabilities)
    ]
    decision_code = (
        AcquisitionDecisionCode.NO_BUDGET_FIT if eligible_lanes else AcquisitionDecisionCode.NO_AUTHORIZED_LANE
    )
    return build(
        None,
