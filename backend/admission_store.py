...
async def _snapshot(db: Any, *, window_start: int, subject_fingerprint: str, now: int) -> AdmissionSnapshot:
...
async def _find_replay(db: Any, *, event_id: str, window_start: int):
    return await db.prepare(
        """SELECT subject_fingerprint, route
           FROM public_admission_events
           WHERE event_id = ? AND window_start = ?
           LIMIT 1"""
    ).bind(event_id, window_start).first()
...
class D1AdmissionStore:
    ...
    async def acquire(
        ...
        *,
        ...
        existing = await _find_replay(self.db, event_id=event_id, window_start=window_start)
        if existing:
            existing_subject = existing.get("subject_fingerprint", "") if isinstance(existing, dict) else getattr(existing, "subject_fingerprint", "")
            existing_route = existing.get("route", "") if isinstance(existing, dict) else getattr(existing, "route", "")
        ...
        inserted = await _insert_if_admissible(
            self.db,
            event_id=event_id,
            window_start=window_start,
            subject_fingerprint=subject_fingerprint,
