...
from .planner_models import FieldRequirement, PaginationKind
...
class PaginationPlan:
    ...
    def bounded(self) -> "PaginationPlan":
        ...
        return PaginationPlan(
            ...
            self.expected_total,
            pages,
            self.detect_repeated_pages,
            self.detect_cursor_repetition,
...
def choose_routes(fields: Sequence[FieldRequirement], routes: Sequence[RepresentationRoute]) -> tuple[RepresentationRoute, ...]:
    ...
    for route in routes:
        ...
        preferred = sum(1 for f in fields if route.representation.value in f.preferred_representations)
...
__all__ = ["FieldRequirement", "PaginationPlan", "Representation", "RepresentationRoute", "choose_routes"]
