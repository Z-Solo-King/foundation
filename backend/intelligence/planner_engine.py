...
def apply_source_profiles(methods: Sequence[MethodCandidate], profiles: Mapping[str,SourceProfileHint])->tuple[MethodCandidate,...]:
    ...
    for method in methods:
        ...
        if profile is None: updated.append(method); continue
        if profile.supported_representations and method.representation not in profile.supported_representations: continue
        expected=min(.99,max(.01,(method.expected_success+profile.health)/2)); expected=(expected+.5)/2 if profile.sample_size<5 else expected
...
def build_task_plan(question: str, output_type: str="", claims: Sequence[str]|None=None, fields: Sequence[FieldRequirement]|None=None, languages: Sequence[str]=(), source_families: Sequence[str]=(), methods: Sequence[MethodCandidate]=(), envelope: ResourceEnvelope|None=None, max_queries: int=12)->TaskPlan:
    mode=classify_task(question,output_type); claim_reqs=decompose_claims(question,claims); field_reqs=normalize_fields(fields); queries=generate_query_portfolio(question,claim_reqs,languages,source_families,max_queries); ranked=apply_source_profiles(methods,{})
...
__all__=["classify_task","infer_fact_type","decompose_claims","normalize_fields","generate_query_portfolio","method_utility","rank_methods","apply_source_profiles","coverage_map","recovery_actions","choose_stop_reason","build_task_plan","create_task_plan"]
