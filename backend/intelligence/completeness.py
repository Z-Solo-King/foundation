...
def certify_completeness(
    pages: tuple[PageObservation, ...],
    *,
    mode: PaginationMode | str,
    termination_reason: str,
    blocked: bool = False,
    ...
    return CompletenessReceipt(
        schema="pagination-completeness-receipt/v1",
        mode=pagination_mode,
        pages_visited=len(pages),
        items_observed=sum(page.item_count for page in pages),
        termination_reason=termination_reason,
        next_page_observed=next_seen,
        completeness=status,
        blocked=blocked,
