(() => {
  ...
  const el = {
    ...
    sidebarOverlay: document.querySelector('.mobile-overlay'),
    conversation: document.getElementById('conversation-scroll'),
  ...
  const renderSources = (sources = []) => sources.map((source, index) => `
    <div class="source-row">
      <span>${index + 1}</span>
      <a href="${api.escapeHtml(source.url || '#')}" target="_blank" rel="noopener noreferrer">${api.escapeHtml(source.title || source.url || 'source')}</a>
      <small>${api.escapeHtml(source.access_state || source.state || source.retrieval_method || 'observed')}</small>
    </div>`).join('');
  ...
  function renderSidebar() {
    ...
    el.chatList.innerHTML = chats.length ? chats.map((chat) => `
      <button class="chat-row ${chat.id === api.state.activeChatId ? 'active' : ''}" data-chat="${api.escapeHtml(chat.id)}" aria-label="Open chat ${api.escapeHtml(chat.title || 'New chat')}" aria-current="${chat.id === api.state.activeChatId ? 'page' : 'false'}">
        <span class="chat-icon">✦</span>
        <span><strong>${api.escapeHtml(chat.title || 'New chat')}</strong><small>${chat.messages?.length || 0} message${(chat.messages?.length || 0) === 1 ? '' : 's'} · ${new Date(chat.createdAt || Date.now()).toLocaleDateString()}</small></span>
      </button>`).join('') : '<div class="sidebar-empty">No matching chats.</div>';
  }
  ...
  function renderConversation() {
    ...
    if (!chat || chat.messages.length === 0) {
      ...
      if (el.emptyState) {
        ...
        el.conversation.append(el.emptyState);
      }
      return;
    }
    ...
    el.conversation.innerHTML = chat.messages.map((message) => {
      ...
      return `<article class="message ${message.role === 'user' ? 'user-message' : 'assistant-message'}" data-message-id="${api.escapeHtml(message.id)}">
        <div class="message-meta"><span>${message.role === 'user' ? 'You' : 'Research AI'}</span><time datetime="${new Date(message.at || Date.now()).toISOString()}">${new Date(message.at || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></div>
        <div class="message-bubble">${api.escapeHtml(message.text)}</div>
        <div class="message-actions">
          <button class="secondary" data-save-message="${api.escapeHtml(message.id)}" aria-label="${saveLabel} message">${saveLabel}</button>
          ${runButton}
        </div>
        ${sources}
      </article>`;
  ...
  function renderProjects() {
    ...
    return `<div class="message view-panel"><div class="view-heading"><div><span class="eyebrow">Workspace</span><h2>Projects</h2><p>Browser-local organization. Projects do not imply a server-backed authority.</p></div><button class="primary" data-action="new-project">＋ New project</button></div>
      <div class="view-grid">${projects.length ? projects.map((project) => {
        const count = api.state.chats.filter((chat) => chat.projectId === project.id).length;
        return `<section class="workspace-card project-card"><div class="card-head"><strong>${api.escapeHtml(project.name)}</strong><span>${count} chat${count === 1 ? '' : 's'}</span></div><div class="card-actions"><button class="secondary" data-project="${api.escapeHtml(project.id)}">Open</button><button class="secondary" data-project-assign="${api.escapeHtml(project.id)}">Use for current chat</button><button class="secondary" data-project-rename="${api.escapeHtml(project.id)}">Rename</button><button class="danger" data-project-delete="${api.escapeHtml(project.id)}">Delete</button></div></section>`;
      }).join('') : '<div class="workspace-card empty-card"><strong>No projects yet</strong><p>Create one to group browser-local chats.</p></div>'}</div></div>`;
  }

  function renderSaved() {
    api.chatStore.repairSavedOwnership();
    const saved = api.state.saved;
    return `<div class="message view-panel"><div class="view-heading"><div><span class="eyebrow">Local library</span><h2>Saved</h2><p>Saved messages stay in this browser.</p></div></div>
      <div class="view-grid">${saved.length ? saved.map((item) => `<section class="workspace-card saved-card"><strong>${api.escapeHtml(item.text)}</strong><small>${new Date(item.at || Date.now()).toLocaleString()}</small><div class="card-actions"><button class="secondary" data-saved-message="${api.escapeHtml(item.messageId)}">Open chat</button><button class="danger" data-remove-saved="${api.escapeHtml(item.messageId)}">Remove</button></div></section>`).join('') : '<div class="workspace-card empty-card"><strong>Nothing saved yet</strong><p>Use Save on a research or chat message.</p></div>'}</div></div>`;
  }

  function renderSettings() {
    const tokenPresent = Boolean(api.token());
    return `<div class="message view-panel"><div class="view-heading"><div><span class="eyebrow">Configuration</span><h2>Settings</h2><p>Browser-local controls for this frontend surface.</p></div></div>
      <section class="workspace-card"><div class="card-head"><strong>Backend</strong><span class="status-dot ${api.state.backendOk ? 'ok' : ''}">${api.escapeHtml(api.state.backendText)}</span></div><p class="mono">${api.escapeHtml(api.API_BASE || 'Not configured')}</p><p class="muted">Session authentication is held only in session storage for this browser tab.</p><label class="field-label" for="session-token">Session token</label><input id="session-token" type="password" placeholder="Optional short-lived token" autocomplete="off" value="${tokenPresent ? '••••••••' : ''}"><div class="card-actions"><button class="secondary" data-action="save-session-token">Use for this tab</button><button class="secondary" data-action="clear-session-token">Clear</button><button class="secondary" data-action="backend-check">Check backend</button></div></section>
      <section class="workspace-card"><div class="card-head"><strong>Local data</strong><span>${api.state.chats.length} chats · ${api.state.projects.length} projects · ${api.state.saved.length} saved</span></div><p class="muted">Exports are versioned local data only. Imports are schema-validated before replacing local state.</p><div class="card-actions"><button class="secondary" data-action="export-data">Export</button><button class="secondary" data-action="import-data">Import</button><button class="danger" data-action="clear-data">Clear local data</button></div></section></div>`;
  }
  ...
  function render() {
    ...
    document.querySelectorAll('[data-view]').forEach((node) => {
      const active = node.dataset.view === api.state.view;
  ...
  document.addEventListener('click', (event) => {
    ...
    if (chat) {
      ...
      api.setActiveChat(chat.dataset.chat);
    ...
    if (nav) {
      event.preventDefault();
      api.state.view = nav.dataset.view;
      closeSidebar();
      render();
      return;
    }
    ...
    if (save) {
      event.preventDefault();
      api.chatStore.saveMessage(save.dataset.saveMessage);
      render();
      return;
    }
    ...
    if (removeSaved) {
      event.preventDefault();
      api.chatStore.removeSaved(removeSaved.dataset.removeSaved);
      render();
      return;
    }
    ...
    if (openSaved) {
      event.preventDefault();
      const chatId = api.chatStore.openSaved(openSaved.dataset.savedMessage);
      if (chatId) { api.state.view = 'chats'; render(); }
      else toast('Saved item has no owning chat.');
      return;
    }
    if (event.target.closest('[data-action="new-project"]')) {
      event.preventDefault();
      const name = window.prompt('Project name');
      if (name) { api.chatStore.createProject(name); render(); }
      return;
    }
  ...
  window.addEventListener('storage', (event) => {
    if ([api.keys.chats, api.keys.projects, api.keys.saved].includes(event.key)) {
      api.load();
      render();
    }
  });
