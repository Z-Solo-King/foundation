(() => {
  ...
  function recentFailures(providers) {
    ...
    return `<ul class="dashboard-list">${failures.slice(0, 30).map(run => `<li><strong>${escapeHtml(run.provider)}</strong> — ${escapeHtml(run.repository || '')} — ${escapeHtml(run.workflow || 'workflow')} — ${escapeHtml(run.conclusion || run.status || 'unknown')} ${safeLink(run.url)}</li>`).join('')}</ul>`;
  }
  ...
  function render(snapshot) {
    ...
    body.innerHTML = `<div class="dashboard-grid">
      ...
      <section class="dashboard-card dashboard-card-wide"><h3>Jobs running</h3>${jobs.length ? `<ul class="dashboard-list">${jobs.map(job => `<li>${escapeHtml(job.provider)} — ${escapeHtml(job.job || job.name || job.job_id || 'job')} — ${escapeHtml(job.status || 'unknown')} ${safeLink(job.url)}</li>`).join('')}</ul>` : '<p class="dashboard-empty">No running jobs reported.</p>'}</section>
      ...
      <section class="dashboard-card dashboard-card-full"><h3>Incidents / bugs / failures</h3>${incidents.length ? incidents.map(item => `<div class="dashboard-incident"><strong>${escapeHtml(item.repository || item.repo || item.provider || '')} ${item.number ? `#${escapeHtml(item.number)}` : ''}</strong> — ${escapeHtml(item.title || item.message || item.code || 'incident')}<div>${safeLink(item.url)}${item.message ? `<span class="dashboard-provenance">${escapeHtml(item.message)}</span>` : ''}</div></div>`).join('') : '<p class="dashboard-empty">No incidents were reported by the configured collectors.</p>'}</section>
    </div>`;
  }
