(() => {
  ...
  if (!api?.workspaceView || !api?.lifecycleStateMachine) {
    throw new Error('frontend_state.js, workspace_view.js and lifecycle_state_machine.js must load before lifecycle_controller.js');
  }
  ...
  const transition = (current, backendValue) => lifecycle.advance(current || 'UNKNOWN', backendValue);

  async function fetchRun(runId) {
    const response = await fetch(`${API_BASE}/api/v1/research/${encodeURIComponent(runId)}`, { headers: headers(false) });
    const body = await response.json().catch(() => ({}));
    if (!response.ok || body.ok === false) {
      const error = new Error(body.error || `Run lookup failed (${response.status})`);
      if (response.status === 401 || response.status === 403) error.code = 'AUTH_EXPIRED';
      throw error;
    }
    return body;
  }
  ...
  async function track(runId, chatId) {
    ...
    while (Date.now() - started < MAX_POLL_MS) {
      ...
      try {
        ...
        const nextUiState = transition(current.ui_state, lifecycle.fromBackend(body));
        ...
        if (terminal(body.status)) {
          ...
          setStatus(`Research ${body.status}; backend state recorded as ${nextUiState}.`);
          document.dispatchEvent(new CustomEvent('rie:chat-updated', { detail: { chatId, runId, final: true, uiState: nextUiState } }));
          void submitNext();
          return;
        }
        setStatus(`Research ${runId} · ${body.status || 'running'} · ${nextUiState}`);
        delay = BASE_DELAY_MS;
      } catch (error) {
        const backendState = error.code === 'AUTH_EXPIRED' ? 'AUTH_EXPIRED' : 'UNKNOWN';
        const nextUiState = transition(current.ui_state, backendState);
        ...
        if (error.code === 'AUTH_EXPIRED') {
          setStatus('Authentication expired; sign in again to reconnect to the backend run.');
          return;
        }
    ...
    if (current?.run_id === runId) {
      const nextUiState = transition(current.ui_state, 'UNKNOWN');
      setActive({ ...current, status: 'unknown', ui_state: nextUiState, stopped: false, last_error: 'poll timeout; refresh/reconnect required' });
      api.workspaceView.render({ ...current, ui_state: nextUiState }, 'Run state could not be confirmed within the browser polling window. The backend run was not marked failed.');
  ...
  async function submit(item) {
    ...
    const submittingState = transition(item.ui_state || 'NEW_CHAT', 'SUBMITTING');
    setActive({ run_id: item.run_id || null, request_id: item.request_id, chat_id: chatId, status: 'submitting', ui_state: submittingState, created_at: Date.now(), stopped: false });
    ...
    if (!response.ok || !body.ok) {
      const error = new Error(body.error || `Research request failed (${response.status})`);
      if (response.status === 401 || response.status === 403) error.code = 'AUTH_EXPIRED';
      throw error;
    }
    const runId = body.run_id;
    const queuedState = transition(submittingState, lifecycle.fromBackend(body) || 'QUEUED');
    setActive({ run_id: runId, request_id: item.request_id, chat_id: chatId, status: body.status || 'submitted', ui_state: queuedState, created_at: Date.now(), stopped: false });
    api.openWorkspace?.();
    api.workspaceView.render(body);
  ...
  function enqueue(text, chatId = currentChatId()) {
    const item = { id: api.uuid(), request_id: api.uuid(), text: String(text || '').trim(), chat_id: chatId, source_urls: [], created_at: Date.now(), ui_state: 'QUEUED' };
    if (!item.text) return;
  ...
  function recover() {
    ...
    const reconnectingState = transition(current.ui_state, 'RECONNECTING');
    setActive({ ...current, status: 'reconnecting', ui_state: reconnectingState });
    api.openWorkspace?.();
  ...
  document.addEventListener('rie:composer-send', (event) => {
    ...
    void submit({ id: api.uuid(), request_id: api.uuid(), text, chat_id: currentChatId(), source_urls: [], created_at: Date.now(), ui_state: 'NEW_CHAT' }).catch((error) => {
      const uiState = error.code === 'AUTH_EXPIRED' ? 'AUTH_EXPIRED' : 'UNKNOWN';
      api.addMessage('assistant', `Research could not be submitted: ${error.message || error}`, { error: true, ui_state: uiState }, currentChatId());
      api.chatView.render();
      setStatus(error.code === 'AUTH_EXPIRED' ? 'Authentication expired; sign in again to submit research.' : 'Research submission failed; backend truth is unchanged.');
    });
  });
  ...
  document.addEventListener('click', (event) => {
    ...
    if (openRun) {
      ...
      setActive({ run_id: runId, request_id: item?.meta?.request_id || api.uuid(), chat_id: chat?.id || currentChatId(), status: 'reconnecting', ui_state: 'RECONNECTING', created_at: Date.now(), stopped: false });
      recover();
      return;
    }
    ...
    if (event.target.closest('[data-lifecycle-stop]')) {
      ...
      const current = active();
      if (current) {
        setActive({ ...current, stopped: true, status: 'paused', ui_state: 'RECONNECTING' });
        api.workspaceView.render({ run: current, status: 'paused', ui_state: 'RECONNECTING', observations: [], capabilities: {} });
