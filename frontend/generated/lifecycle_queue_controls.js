...
(() => {
  ...
  const readQueue = () => {
    try {
      ...
      return Array.isArray(value)
        ? value.filter((item) => item && item.id && item.request_id && item.text)
        : [];
    } catch {
      return [];
    }
  ...
  function render() {
    ...
    if (queueList) {
      queueList.innerHTML = items.length
        ? items.map((item, index) => `<div class="queue-item"><b>${index + 1}</b><div><strong>Research</strong><p>${api.escapeHtml(item.text)}</p></div><button data-lifecycle-remove-queue="${api.escapeHtml(item.id)}" aria-label="Remove queued research">×</button>`).join('')
        : '<div class="queue-empty">Queue is empty.</div>';
    }
  }
  function open() { panel?.classList.add('open'); panel?.setAttribute('aria-hidden', 'false'); overlay?.classList.add('show'); render(); }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (remove) { event.preventDefault(); event.stopPropagation(); document.dispatchEvent(new CustomEvent('rie:queue-remove-requested', { detail: { id: remove.dataset.lifecycleRemoveQueue } })); return; }
