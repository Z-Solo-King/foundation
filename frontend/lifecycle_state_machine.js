(() => {
  ...
  const STATES = Object.freeze([
    'NEW_CHAT','SUBMITTING','QUEUED','RUNNING','STREAMING','COMPLETE','PARTIAL','BLOCKED','REJECTED','UNAVAILABLE','UNKNOWN','RECONNECTING','RESUMED','REPLAYED','AUTH_EXPIRED',
  ]);
  const BACKEND_TO_UI = Object.freeze({
    new:'NEW_CHAT',new_chat:'NEW_CHAT',submitting:'SUBMITTING',submitted:'QUEUED',queued:'QUEUED',running:'RUNNING',streaming:'STREAMING',complete:'COMPLETE',completed:'COMPLETE',success:'COMPLETE',succeeded:'COMPLETE',partial:'PARTIAL',blocked:'BLOCKED',rejected:'REJECTED',unavailable:'UNAVAILABLE',unknown:'UNKNOWN',reconnecting:'RECONNECTING',resumed:'RESUMED',replayed:'REPLAYED',auth_expired:'AUTH_EXPIRED',unauthorized:'AUTH_EXPIRED',forbidden:'AUTH_EXPIRED',paused:'RECONNECTING',
  });
  ...
  const normalize = (value) => { const key=String(value || '').trim().toLowerCase().replace(/[-\s]+/g,'_'); return BACKEND_TO_UI[key] || null; };
  const canTransition = (current,next) => STATES.includes(current) && STATES.includes(next) && (current===next || ALLOWED[current].has(next));
  const advance = (current,backendValue) => { const next=normalize(backendValue); if(!next) return 'UNKNOWN'; if(!current) return next; return canTransition(current,next) ? next : 'UNKNOWN'; };
  const fromBackend = (body) => { const source=body && typeof body==='object' ? body : {}; const raw=source.ui_state || source.state || source.status || source.run?.ui_state || source.run?.state || source.run?.status; return normalize(raw) || 'UNKNOWN'; };
  window.RIEFrontend=window.RIEFrontend || {};
  window.RIEFrontend.lifecycleStateMachine=Object.freeze({STATES,normalize,canTransition,advance,fromBackend});
})();
