...
assert.deepEqual(machine.STATES, [
  'NEW_CHAT', 'SUBMITTING', 'QUEUED', 'RUNNING', 'STREAMING', 'COMPLETE',
  'PARTIAL', 'BLOCKED', 'REJECTED', 'UNAVAILABLE', 'UNKNOWN', 'RECONNECTING',
  'RESUMED', 'REPLAYED', 'AUTH_EXPIRED',
]);
...
assert.equal(machine.canTransition('RUNNING', 'COMPLETE'), true);
assert.equal(machine.canTransition('COMPLETE', 'RUNNING'), false);
assert.equal(machine.canTransition('QUEUED', 'garbage'), false);

assert.equal(machine.advance(null, 'running'), 'RUNNING');
assert.equal(machine.advance('RUNNING', 'streaming'), 'STREAMING');
assert.equal(machine.advance('STREAMING', 'completed'), 'COMPLETE');
...
assert.equal(machine.fromBackend({ state: 'partial' }), 'PARTIAL');
assert.equal(machine.fromBackend({ ui_state: 'RECONNECTING' }), 'RECONNECTING');
assert.equal(machine.fromBackend({}), 'UNKNOWN');
assert.equal(machine.fromBackend(null), 'UNKNOWN');
