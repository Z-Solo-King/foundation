...
const TARGETS = [
  ...
  ["PC Studio","https://www.pcstudio.in"],["PCHubShop","https://www.pchubshop.com"],["Prime ABGB","https://www.primeabgb.com"],["quickincomputers","https://quickincomputers.com"],
  ["SCL Gaming","https://sclgaming.in"],["Variety Infotech","https://varietyinfotech.com"],["Viper PC","https://viperpc.in"],["Moskeys","https://moskeys.com"],
  ["Ninja Dog","https://ninjadog.in"],["Theproaudio","https://www.theproaudio.com"]
];
...
async function commandGet(command, args, timeout=20000) {
  try {
    const { stdout, stderr } = await execFileAsync(command, args, { timeout, maxBuffer: 8 * 1024 * 1024 });
    return { ok:true, text:String(stdout||""), stderr:String(stderr||"") };
  } catch (e) {
    return { ok:false, text:String(e.stdout||""), stderr:String(e.stderr||""), error:e?.code||e?.name||String(e) };
  }
}

async function transportJson(url) {
  const common = ["--max-time","20","--connect-timeout","8","--retry","1","--retry-delay","1","-sS","-L","-4","-A",UA,"-H","Accept: application/json"];
  const variants = [
    ["curl",[...common,url]],
    ["curl",[...common,"--http1.1",url]]
  ];
  for (const [cmd,args] of variants) {
    const r = await commandGet(cmd,args,25000);
    if (!r.ok || !r.text) continue;
    const txt=r.text.trim();
    try { return {status:200,text:txt,via:cmd}; } catch {}
  }
  const w = await commandGet("wget",["-qO-","--timeout=20","--tries=2","--user-agent="+UA,url],25000);
  if (w.ok && w.text) return {status:200,text:w.text.trim(),via:"wget"};
  return null;
}
...
async function reconstructStoreApi(base, out) {
  ...
  if (!products.length) {
    for (const endpoint of candidates) {
      let page = 1;
      const batch = [];
      while (page <= 250 && batch.length < MAX_PRODUCTS) {
        const url = new URL(endpoint.href);
        url.searchParams.set("page", String(page));
        url.searchParams.set("per_page", String(PAGE_SIZE));
        url.searchParams.set("orderby", "id");
        url.searchParams.set("order", "asc");
        const tr = await transportJson(url.href);
        if (!tr?.text) break;
        let data;
        try { data = JSON.parse(tr.text); } catch { break; }
        if (!Array.isArray(data) || !data.length) break;
        batch.push(...data);
      ...
      if (batch.length) {
        chosen = endpoint.href + " [transport]";
  ...
  out.store_api = {
    ...
    pages_fetched: products.length ? Math.ceil(products.length/PAGE_SIZE) : 0,
...
async function robotsAndSitemaps(base,out) {
  try {
    ...
    out.sitemaps=[...new Set(urls)].slice(0,12);
    for(const s of out.sitemaps) {
      try {
        const sr=await get(s);
        if(sr.status!==200 || !sr.text) continue;
        for(const m of String(sr.text).matchAll(/https?:\/\/[^\s<"']+/gi)) {
          const u=m[0].replace(/[)>,"']+$/,"");
          if(sameOrigin(u,base) && /(google|merchant|shopping|feed|product|woo-feed|wppfm|codesolz|feedcraft)/i.test(u)) {
            out.robotFeedCandidates ??= [];
            if(!out.robotFeedCandidates.includes(u)) out.robotFeedCandidates.push(u);
          }
        }
      } catch {}
    }
...
async function commonHistory(base) {
  try {
    const ci=await get("https://index.commoncrawl.org/collinfo.json");
    if(ci.status!==200 || !ci.text) return [];
    const info=JSON.parse(ci.text);
    const collection=Array.isArray(info)?String(info[0]?.id||""):"";
    if(!collection) return [];
    const host=new URL(base).hostname;
    const r=await get(`https://index.commoncrawl.org/${encodeURIComponent(collection)}-index?url=${encodeURIComponent(host+"/*")}&output=json`);
    if(r.status!==200 || !r.text) return [];
    const urls=[];
    for(const line of r.text.split(/\r?\n/)) {
      try {
        const obj=JSON.parse(line), u=obj?.url||"";
        if((/\.xml(?:[?#]|$)/i.test(u)||/(google|merchant|shopping|product-feed|woo-feed|wppfm|feedcraft|codesolz)/i.test(u)) && !/\/product\//i.test(u)) urls.push(u);
      } catch {}
