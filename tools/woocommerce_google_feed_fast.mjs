...
async function scan([name,base]){
 ...
 if(out.feeds.length===0){
  let i = 0;
  const urls = [...new Set(cand)];
  async function worker() {
    while (i < urls.length && out.feeds.length === 0) {
      const u = urls[i++];
      try {
        const r = await get(u);
        if (r.status === 200 && r.text) {
          ...
          out.probes.push({
            url: u,
            status: r.status,
            classification: v.qualifies ? "LIVE_VERIFIED" : "NO_MATCH",
            ...v
          });
          if (v.qualifies) {
          ...
          let classification = `HTTP_${r.status}`;
          if (challenge(r.status, r.text)) classification = "BLOCKED_OR_CHALLENGED";
          else if (r.status === 401) classification = "AUTH_REQUIRED";
          else if (r.status === 403) classification = "ACCESS_DENIED";
          else if (r.status === 429) classification = "RATE_LIMITED";
          out.probes.push({ url: u, status: r.status, classification });
        }
      } catch (e) {
        out.probes.push({
          url: u,
