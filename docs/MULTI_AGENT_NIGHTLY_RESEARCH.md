...
Before executing research, each Foundation lane uses the purpose-specific GitHub App installation credential to obtain a short-lived read token and checks out the exact immutable Operations revision recorded in `.github/workflows/nightly-multi-agent-research.yml`.
