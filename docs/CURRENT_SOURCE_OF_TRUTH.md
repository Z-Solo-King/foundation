...
This is the current cross-surface checkpoint. Fresh GitHub, Cloudflare, workflow, artifact and D1 evidence outrank all dated sections below.
...
- Open issues: Foundation 7 + Operations 12 = 19.
- Open PRs: Foundation 8 + Operations 4 = 12.

## Cloudflare
- Canonical path: Pages `ai` (`ai-cio.pages.dev`) -> Worker `heroic` v30 -> private Worker `operations` v90.
- `heroic` provenance: Foundation `ee312632fce5be5da9536194e94f38fa2cdabb6b`.
- `operations` provenance: production pin `566fe7b90c15a8e0ad8210bd98a7b514de6f5fc3`.
- D1: `research-intelligence`.
- `heroic-ai.dev`: pending/unresolvable; it is not the current Pages hostname.
- Legacy `foundation` Worker exists but is not the canonical public backend.

## Evidence
- Nightly #375 / run `36141145555`: blocked before provider execution; 24 program slots prepared; no provider-backed acceptance.
- Migration review: 40 matrix cases + 24 capacity cases; score 60/100; deterministic-structural evidence.
- Extractor benchmark #448 / run `36141127492`: artifact `10867020802`; 40 receipts = 4 ok / 32 empty / 4 blocked; structural/integrity pass only.
- Direct D1: 707 research runs (484 completed / 223 planned), 440 observations, 0 publications, 581 reservations, 135 quota rows, 1,436 chat-idempotency rows.

## Continuity
- Both Custom GitHub and Custom Cloudflare connectors worked in this project chat on 2026-09-26.
- Canonical source-of-truth and family-sync documents are checkpoints, not production evidence.
- Do not create another competing “current” snapshot.
