...
This record reconciles the current GitHub repository state with the older project plans and handoff documents discussed during the AI ChatBot / Heroic AI / Research Intelligence Engine work.
...
The previously referenced standalone `extractor-mapper` repository is historical context and is not part of the current active two-repository synchronization boundary. Older documents that describe three active repositories must therefore be treated as historical, not current architecture.

## Current observed branch tips

- Foundation `main`: `a8ae322110a79cd93a2f71bdf38d72bf70a4321b`
- Operations `main`: `7415c55f0c23eb8a3d57725611b6e741a1dc2453`
...
2. Operations `docs/CURRENT_SOURCE_OF_TRUTH.md` has been reconciled to the current two-repository boundary and current observed branch tips.
3. Foundation `docs/FAMILY_SYNC_STANDARD.md` remains the normative synchronization format and evidence hierarchy.
4. Foundation `docs/FAMILY_SYNC_STATE.json` remains the machine-readable family snapshot and must be updated through the protected PR/check path.
5. Current issue/PR state is authoritative over copied issue counts in older handoffs. The GitHub UI currently reports 112 open authored issues and 24 open authored PRs for the user; individual repository/API results should be used for exact item state.
6. Open PRs are proposed until merged. A merged implementation is `CURRENT`; CI execution is L3; approved private/production runtime is L4.
7. No GitHub repository document may claim Cloudflare/B2/private-runtime production truth from source inspection or repository CI alone.

## Current implementation coordination

### Public admission/security

Foundation PR `#460` consolidates concrete public admission hardening for auth fail-closed behavior, client `system` history rejection, finite request ceilings, JSON representation admission and public exception sanitization. Its latest required-check run reached compilation, package installation and CodeQL successfully but failed at the product-test step. It must remain open until the failure is resolved and required checks pass.

### Stage receipt correctness

Foundation PR `#461` groups stage-receipt expiry and cumulative resource-chain validation. It extends the deterministic receipt guard without becoming a second ResourceLedger authority.

### Provider-output admission

Operations PR `#389` was merged to bound provider responses before downstream use and prevent ungoverned tool/fan-out behavior. Remaining structural fan-out and streaming requirements continue under the existing Operations contracts rather than a new provider authority.
...
- A 100% configured coverage gate is not a 100% execution claim until CI runs and passes it.
