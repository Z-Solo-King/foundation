...
def test_malformed_claims_and_lineage_arrays_are_rejected():
    signed = package()
    with pytest.raises(ValueError, match="claims must be objects"):
        parse_trusted_package({**signed.__dict__, "claims": ({"claim": "x"}, "bad")}, b"test-key")
    with pytest.raises(ValueError, match="arrays"):
        parse_trusted_package({**signed.__dict__, "source_versions": "v1"}, b"test-key")
