...
class FakeStatement:
    ...
    async def run(self):
        return self.value or {"success": True}
...
async def test_search_contract_and_authorization():
    ...
    async def implementation(query, limit):
        return [SearchResult("https://example.com", query, "snippet", "family")][:limit]

    results = await search("query", 1, authorization, implementation)
    assert results[0].title == "query"
    with pytest.raises(ValueError):
        await search("", 1, authorization, implementation)
    with pytest.raises(ValueError):
        await search("q", 0, authorization, implementation)
    with pytest.raises(PermissionError):
        await search("q", 1, SearchAuthorization("x", False, True, True), implementation)
    with pytest.raises(PermissionError):
        await search("q", 1, SearchAuthorization("x", True, False, True), implementation)
    with pytest.raises(PermissionError):
        await search("q", 1, SearchAuthorization("x", True, True, False), implementation)
...
async def test_public_http_validation_and_redirects():
    ...
    for url in ("ftp://example.com", "http://127.0.0.1", "http://localhost", "https://user:pass@example.com", "https://example.com:8443"):
        with pytest.raises(ValueError):
            source_http.validate_url(url)

    responses = iter([
        FakeHTTPResponse(302, {"location": "/next"}),
        FakeHTTPResponse(200, {"content-type": "text/plain", "etag": "e1"}, b"hello"),
    ])
...
async def test_cloudflare_persistence_helpers():
    ...
    env = SimpleNamespace(DB=db, ARTIFACTS=artifacts)
    persistence = CloudflarePersistence(env)

    assert await persistence.create_run("run-1", request) == "run-1"
    with pytest.raises(ValueError):
        await persistence.create_run_idempotent(request, "")
    ...
    with pytest.raises(IdempotencyConflictError):
        await persistence.create_run_idempotent(request, "key-1")

    db.batch_result[2].results = []
    with pytest.raises(RuntimeError, match="not persisted"):
        await persistence.create_run_idempotent(request, "key-2")

    await persistence.get_run("run-1")
    with pytest.raises(ValueError):
        await persistence.set_run_status("run-1", "bad")
    await persistence.set_run_status("run-1", "running")
    result = await persistence.put_artifact("k", b"abc", "text/plain")
    assert result["size"] == 3
    assert await persistence.get_artifact("k") == b"abc"
    assert await persistence.get_artifact("missing") is None


def test_d1_repository_all_paths():
    repo = D1Repository()
    now = datetime.now(timezone.utc)
    run = RunRecord("r1", "q", "standard", "planned", now, now, 1, 2, "{}")
    assert repo.create_run(run) is run
    with pytest.raises(ValueError):
        repo.create_run(run)
    ...
    evidence = EvidenceRecord("e1", "r1", "o1", "s1", None, 0, 3, "hash", now)
    assert repo.add_evidence(evidence) is evidence
    with pytest.raises(ValueError):
        repo.add_evidence(evidence)
    assert repo.evidence_for_run("r1") == [evidence]
    assert repo.evidence_for_claim("missing") == []
    lineage = SourceLineageRecord("s1", "f1", None, "origin", now, now)
    assert repo.upsert_lineage(lineage) is lineage
    assert repo.get_lineage("s1") is lineage
    assert repo.get_lineage("missing") is None
    assert repo.lineage_for_family("f1") == [lineage]
    version = DocumentVersionRecord("v1", "o1", "s1", now, None, "hash", "artifact")
    assert repo.add_version(version) is version
    with pytest.raises(ValueError):
...
def test_r2_repository_all_paths():
    now = datetime.now(timezone.utc)
    repo = R2Repository()
    artifact = R2Artifact("a1", "project-artifacts", "k", "text/plain", 3, now, "hash")
    assert repo.upload(artifact, b"abc") is artifact
    with pytest.raises(ValueError):
        repo.upload(artifact, b"abc")
    with pytest.raises(ValueError):
        repo.upload(R2Artifact("a2", "project-artifacts", "k2", "text/plain", 4, now, "hash"), b"abc")
    assert repo.download("a1")[1] == b"abc"
    assert repo.download("missing") is None
    repo.delete("a1")
    repo.delete("missing")
    manifest = ArtifactManifest("m1", "name", "desc", "text", {"encoding": "utf-8"})
    assert repo.add_manifest(manifest) is manifest
    with pytest.raises(ValueError):
        repo.add_manifest(manifest)
    assert repo.get_manifest("m1") is manifest
    assert repo.get_manifest("missing") is None
...
async def test_worker_helpers_and_source_ingestion(monkeypatch):
    ...
    assert worker._request_hash({"a": 1}) == worker._request_hash({"a": 1})
    ...
    db = FakeDB()
    artifacts = FakeArtifacts()
    env = SimpleNamespace(DB=db, ARTIFACTS=artifacts, ENVIRONMENT="development", AUTH_TOKEN=None)
    await worker._persist_run(env, "run-1", request, "hash", "idem")

    async def fake_public(url):
        return source_http.FetchResult(url, url, 200, "text/plain", b"abc", "e")
