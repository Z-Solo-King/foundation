...
def test_regression_when_execution_degrades():
    result = compare(
        _summary(completed=1, useful=1, execution="partial_or_failed"),
        _summary(completed=2, useful=4, execution="completed"),
    )
...
def test_regression_wins_when_mixed_signals_conflict():
    result = compare(
        _summary(completed=3, useful=8, high=3),
        _summary(completed=2, useful=4, high=1),
    )
    assert result["decision"] == "REGRESSED"
...
def test_improvement_when_execution_recovers():
    result = compare(
        _summary(completed=2, useful=4, high=0, execution="completed"),
        _summary(completed=1, useful=4, high=1, execution="partial_or_failed"),
    )
...
def test_improvement_when_useful_findings_increase():
    result = compare(
        _summary(completed=2, useful=5, high=1),
        _summary(completed=2, useful=4, high=1),
    )
    assert result["decision"] == "IMPROVED"
...
def test_stable_when_metrics_are_equal():
    result = compare(
        _summary(completed=2, useful=4, high=1, signals=[{"type": "gap", "severity": "high"}]),
        _summary(completed=2, useful=4, high=1, signals=[{"type": "gap", "severity": "high"}]),
    )
...
def test_signal_type_changes_are_recorded():
    result = compare(
        _summary(signals=[{"type": "new", "severity": "medium"}]),
        _summary(signals=[{"type": "old", "severity": "medium"}]),
    )
...
def test_summary_with_missing_signal_lists_is_supported():
    current = _summary(signals=None)
    previous = _summary(signals=None)
