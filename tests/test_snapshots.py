...
def test_answer_snapshot_captures_missing_contradiction_provider_and_reliability_dimensions():
    previous = answer()
    current = AnswerSnapshot(
        **{
            **previous.__dict__,
            "missing_scope": ("q2",),
            "contradiction_state": {"c1": "none"},
            "provider_model_identity": "groq/model-v2",
            "reliability_state": "stable",
        }
    )
    diff = previous.semantic_diff(current)
    assert diff["scope_changed"] is True
    assert diff["contradiction_state_changed"] is True
    assert diff["provider_model_changed"] is True
    assert diff["reliability_changed"] is True


def test_answer_snapshot_rejects_blank_provider_or_reliability_identity():
    previous = answer()
    with pytest.raises(ValueError, match="provider_model_identity"):
        AnswerSnapshot(**{**previous.__dict__, "provider_model_identity": " "}).validate()
    with pytest.raises(ValueError, match="reliability_state"):
        AnswerSnapshot(**{**previous.__dict__, "reliability_state": " "}).validate()
