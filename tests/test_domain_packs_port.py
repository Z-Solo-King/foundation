...
def test_keyboard_domain_rules_cover_all_rule_kinds():
    text = (
        "tkl PBT hot-swap prebuilt 1.25 kg 3000 mAh 3-pin knob "
        "gasket abs 1000 Hz bluetooth hall effect"
    )
...
def test_mouse_and_display_rules_cover_regex_measurement_dpi_and_maps():
    mouse = apply_domain_rules(
        "BAMF 3.0 68 g 2000 mAh 26K DPI bluetooth omron palm",
        ["mouse"],
    )
...
def test_mousepad_rules_and_special_weight():
    pad = apply_domain_rules(
        "mousepad very fast textured very soft 4 mm",
        ["mousepad"],
    )
    assert pad == {
...
def test_first_rule_wins_and_device_text_override():
    out = apply_domain_rules(
        "1 kg 1000 Hz",
        ["keyboard", "mouse"],
        device_text="TKL 1 kg 1000 Hz",
    )
