...
def test_zero_job_complete_graph_is_rejected():
    with pytest.raises(ValueError, match="zero-job"): receipt(job_count=0, graph_state="COMPLETE", job_state="UNKNOWN")
...
def test_negative_job_count_is_rejected():
    with pytest.raises(ValueError, match="negative"): receipt(job_count=-1)
def test_invalid_schema_is_rejected():
    with pytest.raises(ValueError, match="unsupported"): receipt(schema="bad")
