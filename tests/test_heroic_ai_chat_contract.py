...
def test_chat_request_rejects_invalid_modes_paid_execution_and_bounds():
    ...
    valid.validate()
    invalid = (
        ChatRequest("", "request", "hello"), ChatRequest("chat", "", "hello"), ChatRequest("chat", "request", ""),
        ChatRequest("chat", "request", "x" * 16_385), ChatRequest("chat", "request", "hello", mode="research"),
...
def test_public_chat_boundary_uses_internal_service_binding_and_idempotency():
    ...
    assert url == "https://chat/v1/chat" and options["headers"]["Authorization"] == "Bearer token"
    assert options["headers"]["Idempotency-Key"] == "request" and json.loads(options["body"]) == payload
...
def test_public_worker_chat_route_validation_and_fail_closed_paths():
    ...
    class Request:
        def __init__(self, payload=None, headers=None):
            self.method, self.url, self._payload, self.headers = "POST", "https://example/api/v1/chat", payload, headers or {}
    ...
    assert asyncio.run(unauthorized.fetch(Request(payload))).status == 401
    class AuthorizedWorker(worker.Default): pass
    invalid_json = AuthorizedWorker(); invalid_json.env = SimpleNamespace(AUTH_TOKEN=None, ENVIRONMENT="development", LOCAL_DEVELOPMENT_AUTH_BYPASS="true", OPERATIONS=Binding())
    assert asyncio.run(invalid_json.fetch(Request(None))).status == 400
    malformed = AuthorizedWorker(); malformed.env = invalid_json.env
    assert asyncio.run(malformed.fetch(Request({**payload, "mode": "research"}))).status == 400
    unavailable = AuthorizedWorker(); unavailable.env = SimpleNamespace(AUTH_TOKEN=None, ENVIRONMENT="development", LOCAL_DEVELOPMENT_AUTH_BYPASS="true")
    assert asyncio.run(unavailable.fetch(Request(payload))).status == 503
    success = AuthorizedWorker(); success.env = SimpleNamespace(AUTH_TOKEN=None, ENVIRONMENT="development", LOCAL_DEVELOPMENT_AUTH_BYPASS="true", OPERATIONS=Binding())
    assert asyncio.run(success.fetch(Request(payload, {"Idempotency-Key": "request"}))).status == 200
