...
@pytest.mark.asyncio
async def test_worker_private_chatbot_diagnostic_checks_binding_boundary(monkeypatch):
    class Upstream:
        status = 200
    ...
    binding = Binding()
    body, status = await worker._operations_chatbot_diagnostic(SimpleNamespace(OPERATIONS=binding))
    assert status == 200
    assert body["ok"] is True
    assert binding.calls[0][0] == "https://private/v1/diagnostics/chatbot"
    assert binding.calls[0][1]["method"] == "POST"
...
async def test_worker_http_public_diagnostics_and_research_fail_closed_paths(monkeypatch):
    ...
    assert "public_chatbot" in str(public_test)
