...
if curl -fsSL --retry 2 --connect-timeout 5 --max-time 20 \
  -A 'Heroic-AI-nightly-research/1.1' \
...
jq -n \
  ...
  '{
    schema:"nightly-ai-research-observation/v2",
    run_id:$run_id,
    run_attempt:$attempt,
    job_id:$job_id,
    topic:$topic,
...
jq -e '.schema == "nightly-ai-research-observation/v2" and (.job_id|length>0) and (.topic|length>0) and (.target_issues|length>0) and (.research_focus|length>0) and (.evidence_class=="research-signal") and (.hidden_reasoning_recorded==false)' \
...
if [ "$gh_status" -ne 200 ] && [ "$gh_issue_status" -ne 200 ] && [ "$reddit_status" -ne 200 ] && [ "$reddit_fallback_status" -ne 200 ] && [ "$x_status" -ne 200 ]; then
