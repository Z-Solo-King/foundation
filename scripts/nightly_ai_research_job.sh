...
github_issue_query="$(encode "\${SOCIAL_QUERY} updated:>=\${since}")"
...
if curl -fsSL --retry 2 --connect-timeout 5 --max-time 20 \
  -A 'Heroic-AI-nightly-research/1.0' \
...
jq -e '.schema == "nightly-ai-research-observation/v1" and (.job_id|length>0) and (.topic|length>0) and (.target_issues|length>0) and (.research_focus|length>0) and (.evidence_class=="research-signal") and (.hidden_reasoning_recorded==false)' \
...

{
  ...
  echo "- GitHub issues: \${gh_issue_status}"
